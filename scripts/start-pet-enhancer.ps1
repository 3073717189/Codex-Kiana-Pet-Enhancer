...
$Injector = Join-Path $PSScriptRoot 'pet-injector.mjs'
. (Join-Path $PSScriptRoot 'pet-common.ps1')
$productName = 'Codex Kiana Pet Enhancer'
$stateDirectoryName = 'CodexKianaPet'

if ($SelfTest) {
  $node = Get-DreamSkinNodeRuntime
  & $node.Path $Injector --check-payload
...
try {
  ...
  if ($savedIsDifferent) {
    ...
    if ($currentProcesses.Count -gt 0 -and ($savedProcesses.Count -gt 0 -or $savedOwnsPort)) {
      throw 'Multiple registered Codex package versions are active. Close them manually before starting Pet Enhancer.'
    }
    if ($savedProcesses.Count -gt 0 -or $savedOwnsPort) {
      ...
      if ($null -ne $savedIdentity) {
        ...
        Write-Warning 'Reapplying Pet Enhancer to the still-running registered Codex version; the current Store version will be used after that app exits.'
      } else {
        $codexToStop = $savedCodex
  ...
  if (-not $debugReady -and $codexProcesses.Count -gt 0) {
    ...
    if (-not $restartAuthorized -and $PromptRestart) {
      $restartAuthorized = Confirm-DreamSkinRestart -Message "$productName needs to restart Codex once. Unsaved input may be lost. Restart now?"
      if (-not $restartAuthorized) {
        Write-Host "$productName launch was cancelled; Codex was not changed."
        exit 0
      }
    }
    if (-not $restartAuthorized) {
      throw "Codex is open without a verified $productName CDP endpoint. Close it first or explicitly use -RestartExisting."
    }
  ...
  try {
    if ($null -eq (Get-DreamSkinVerifiedCdpIdentity -Port $Port -Codex $codex)) {
      if (-not (Test-DreamSkinPortAvailable -Port $Port)) {
        ...
        Write-Warning "Preferred loopback port $preferredPort is unavailable; using $Port."
      }
      ...
      Start-DreamSkinCodex -Codex $codex -Arguments $arguments
    ...
    if (($closedExistingCodex -or $launchedWithCdp) -and
      ...
      if ($launchedWithCdp) {
        Write-Warning "$productName launch failed; reopening Codex without a debugging port."
      }
  ...
  try {
    ...
    if ($verifyExitCode -ne 0) {
      $verifySummary = (($verifyOutput | ForEach-Object { "$_" }) -join "`r`n").Trim()
      if ($verifySummary.Length -gt 1600) { $verifySummary = $verifySummary.Substring(0, 1600) + '...' }
      throw "Pet Enhancer verification failed with exit code $verifyExitCode.`r`n$verifySummary`r`nSee $VerifyPath"
    }
  } catch {
    $startupError = $_
  ...
  Write-Host "$productName is active on verified loopback port $Port."
} finally {
